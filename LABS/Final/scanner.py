...
programCode = open("C:/Users/user/Documents/Semester#7/FLACT/LABS/Parser/Data/program.txt", "r")
tokens = open("C:/Users/user/Documents/Semester#7/FLACT/LABS/Parser/Data/token.txt", "r")
        ...
        if(words[0] == "states"):
            ...
            for i in range(0, len(temp), 3):
                if((temp[i], temp[i+1]) in transition):
                    # print(transition[(temp[i], temp[i+1])], temp[i], temp[i+1])
...
def main_operations():
    ...
    for line in code_arrayOfLine:
        ...
        for indeex, word in enumerate(line):
            if(word in tokender):
                if(check_operator):
                    # print(line[indeex-1]+word)
                    program_stream += word + " "
                    pif.pop(len(pif)-1)
                    temp = line[indeex-1]+word
                    pif.append({temp: tokender[temp], 'ans': "-1"})
                    check_operator = False
                    continue
            ...
            if(word in operators):
                if(str(word + line[indeex+1]) in operators):
                    program_stream = program_stream[:-2]
                    program_stream += word
                    check_operator = True
                    continue
                else:
            ...
            if(checkIdentifier == True and isOkInit == True and word != ''): #check if name is acceptable for identifier
            # if(checkIdentifier == True and isOkInit == True and word != '' or word.find(".") != -1): #check if name is acceptable for identifier
                ...
                if(checkFAConst(initial_const, 0, word)):
                    ...
                    errors.append([word, "Error appeared. Non clear word-element. Line:", code_arrayOfLine.index(line)])
                    print(errors, "Lexical error")
                    exit(0)
    return program_stream[:-1]
...
get_codes_array()
program_stream = main_operations()
# print(program_stream)
